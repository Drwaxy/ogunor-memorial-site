...
function githubClient(env, fetcher) {
  const repo = (env.CONDOLENCE_GITHUB_REPO || '').trim();
  const token = (env.CONDOLENCE_GITHUB_TOKEN || '').trim();
  if (!/^[\w.-]+\/[\w.-]+$/.test(repo) || !token) throw storageError('storage_configuration');
  const base = `https://api.github.com/repos/${repo}`;
  async function api(path, method = 'GET', body) {
    const response = await fetcher(base + path, { method, headers: { Authorization: `Bearer ${token}`, Accept: 'application/vnd.github+json', 'Content-Type': 'application/json', 'X-GitHub-Api-Version': '2022-11-28' }, ...(body ? { body: JSON.stringify(body) } : {}), signal: AbortSignal.timeout(12000) });
    if (!response.ok) throw storageError(path.startsWith('/contents/') ? 'github_receipt' : (path.startsWith('/git/') ? 'github_' + path.split('/')[2] : 'github_repository'), response.status);
    return response.json();
  }
  return api;
}
async function checkStorage(env = process.env, fetcher = fetch) {
  const api = githubClient(env, fetcher);
  const info = await api('');
  if (info.private !== true) throw storageError('repository_not_private');
  await api(`/git/ref/heads/${encodeURIComponent(info.default_branch)}`);
}
...
module.exports = { validate, filesFor, archive, checkStorage, MAX_BODY };
