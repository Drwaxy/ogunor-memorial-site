const { validate, archive, MAX_BODY } = require('../lib/condolence-store.cjs');
...
module.exports = async function handler(req, res) {
  ...
  if (req.method === 'GET') return reply(200, { preview: false, siteKey: env.TURNSTILE_SITE_KEY });
  ...
  try {
    const response = await fetch('https://challenges.cloudflare.com/turnstile/v0/siteverify', { method: 'POST', body: new URLSearchParams({ secret: env.TURNSTILE_SECRET_KEY, response: body.token }), signal: AbortSignal.timeout(10000) });
  ...
  } catch { return reply(503, { error: 'Unable to save. Please retry.' }); }
};
